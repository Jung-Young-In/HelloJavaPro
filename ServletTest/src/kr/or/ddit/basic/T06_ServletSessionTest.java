 ...
 * boolean값 : true인 경우 => 세션 객체가 존재하지 않으면 새로 생성함.
 * 			  false인 경우 => 세션 객체가 존재하지 않으면 null 리턴함.
 * 
 * - 세션 삭제 처리 방법
 * 1. invalidate() 메서드 호출
...
public class T06_ServletSessionTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		HttpSession session = req.getSession(true);	//미 입력시 default값이 true(명시적으로 하고 싶다면 false)
		...
		String userId = "mattD";	//사용자 ID
		
		if(session.isNew()) {	//새로 만든 세션인 경우
			title = "처음 방문하신 것을 환영합니다.";
