...
/**
 * 쿠키 정보를 다루기 위한 예제
 * (웹 서버와 브라우저는 애플리케이션을 사용하는 동안 필요한 값을 쿠키를 통해
 *  공유하여 상태를 유지함)
 * 1. 구성 요소
 * - 이름
 * - 값
 ...
 *      쿠키를 저장(생성)하지 않는다.(보안 문제 때문) 
 * 
 * 2. 동작방식
 * - 쿠키 생성단계 : 생성한 쿠키를 응답데이터의 헤더에 저장하여 웹브라우저에 
 * 				전송한다.
 * - 쿠키 저장단계 : 브라우저는 응답데이터에 포함된 쿠키를 쿠키 저장소에 보관한다. 
 * 				(쿠키 종류에 따라 메모리나 파일에 저장함)
...
//		readCookieExam(req, resp);		//쿠키 정보 읽기 예제
	 ...
	 * 	  => 이외의 값(예를 들면 한글)을 사용 시에는 URLEncoder.encode() 
	 *       사용하여 인코딩 처리를 해준다.
	 ...
	 */
		...
		//쿠키 추가하기(톰캣이 나중에 헤더에 setcookie를 설정해주는 부분)
